...
Uses matplotlib + reportlab-free approach with FPDF2.
Falls back to basic image-based PDF if dependencies missing.
"""
...
def generate_pdf(metrics_dir):
    """Generate REPORT.pdf from REPORT.md, analysis.md, and chart images."""
    ...
    for label, img_path in charts:
        ...
        pdf.cell(0, 10, f"Chart: {label}", ln=True)
        pdf.ln(2)
        try:
            ...
            pdf.cell(0, 8, f"(Image load error: {e})", ln=True)
    ...
    if analysis:
        pdf.add_page()
        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 10, "Bottleneck Analysis & Recommendations", ln=True)
        pdf.ln(4)
        ...
        for line in analysis.split("\n"):
            ...
            if stripped.startswith("### "):
                ...
                pdf.cell(0, 7, stripped.replace("### ", ""), ln=True)
                pdf.set_font("Helvetica", "", 9)
            elif stripped.startswith("## "):
                pdf.ln(6)
                pdf.set_font("Helvetica", "B", 13)
                pdf.cell(0, 8, stripped.replace("## ", ""), ln=True)
                pdf.set_font("Helvetica", "", 9)
                ...
                text = stripped[2:].replace("**", "").replace("[CRITICAL]", "(!!)").replace("[WARNING]", "(!)").replace("[GOOD]", "(OK)").replace("[INFO]", "(i)")
                pdf.cell(5, 6, "")
    ...
    pdf.ln(20)
    pdf.cell(0, 8, "Generated by oci-db-metric-report", ln=True, align="C")
    pdf.cell(0, 8, "https://github.com/jaesucjang/oci-db-metric-report", ln=True, align="C")
